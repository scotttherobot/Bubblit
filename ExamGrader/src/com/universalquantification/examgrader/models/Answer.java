...
/**
 * Answer represents an answer on the exam. It is comprised of a set of bubbles
 * called choices and can be compared and by these choices. An answer also has a
 * number representing which numbered question it is the answer to on a exam. An
 * answer can also be printed for convenience.
 *
 * @author CY Tan
 * @version 2.0
 */
public class Answer implements Comparable<Answer>
{
    ...
    {
        // check every bubble to make sure its filled in.
     ...
     * between the two answers are equal and whether or not the question numbers
     * between the two answers are equal to, greater than, or less than each
     * other.
     * @pre o is not null.
     */
    @Override
    public int compareTo(Answer o)
    {
        ...
        // check that the question numbers are the same
        if (number < otherNumber)
        {
            returnValue = -1;
        }
        // ceck that the numbers are the same
        else if (number > otherNumber)
        {
            returnValue = 1;
        }

        // if the numbers are the same, check the answers.
        if (returnValue == 0)
        {
            ...
            // go therough every bubble and check that they're the same.
            for (int idx = 0; idx < choicesArray.length; idx++)
            {
                // check if the bubbles are the same.
                if (!((Bubble) choicesArray[idx]).equals(
                    (Bubble) otherChoicesArray[idx]))
                {
                    returnValue = -1;
     ...
     *
     * @param o The answer to compare to this answers
     * @return Either true or false depending on whether or not the choices and
     * question number between the two answers are equal.
     * @pre o is not null.
     */
    @Override
    public boolean equals(Object o)
    {
        ...
        // make sure we're comparing the same types of objects
    ...
    public String toString()
    {
        ...
        // go through every bubble to get its string.
        for (Bubble choice : choices)
        {
            // check if the bubble is filled in.
            if (choice.isFilled())
            {
